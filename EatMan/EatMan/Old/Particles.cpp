...
GroupParticles::GroupParticles(float number, float size, float speed, float lifeTime, float posX, float posY, float dirX, float dirY, float spray) {
	...
	for (int i = 0; i < number; i++)
	{
		...
		particles.back()->SetShapeColor(Color(255, 20, 20, 255));
	}
...
GroupParticles::GroupParticles(float number, float sizeMin, float sizeMax, float speedMin, float speedMax, 
	float lifeTimeMin, float lifeTimeMax, float posX, float posY, float dirX, float dirY, float spray)
{
	float ang = atan(dirY / dirX) * RADTODEG;
	if (dirX < 0) {
		ang += 180;
